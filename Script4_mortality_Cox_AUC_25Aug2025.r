...
out4 <- cbind(auc_null, auc_all, auc_p_comp)

out4$Clock <- row.names(out4)
names(out4)[1:3] <- c("Null_AUC","Full_AUC","AUC_P_comp")

out4$AUC_diff <- out4$Full_AUC - out4$Null_AUC
out4 <- out4[,c(4,1,2,5,3)]

out5 <- merge(out3, out4, by="Clock")

write.csv(out5, "/Cluster_Filespace/Marioni_Group/Riccardo/Christos_17Oct2024/Results/Mortality_HR_AUC_25Aug2025.csv", row.names=F)