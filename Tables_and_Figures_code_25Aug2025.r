...
data <- read.csv("Disease_HR_AUC_merged_25Aug2025.csv", header=T)

dis <- read.delim("combined_disease_names_and_groups.txt")
data <- merge(data, dis, by.x="disease", by.y="name")
names(data)[c(1,16)] <- c("Dis_short","Disease")
...
data <- read.csv("Mortality_HR_AUC_25Aug2025.csv", header=T)
...
s5_tmp <- as.data.frame(summary(model)$coefficients[,c(1,2,5)])

s5_tmp$Clock <- rownames(s5_tmp)

names(s5_tmp)[1:3] <- c("Estimate","SE","P") 
...
data <- read.csv("All_Results_22Aug2025.csv", header=T)

dis <- read.delim("combined_disease_names_and_groups.txt")
data <- merge(data, dis, by.x="disease", by.y="name")
names(data)[c(1,12)] <- c("Dis_short","Disease")
...
data <- data[,c(13,12,2:11)]

data <- data[order(data$Group, data$Disease, data$Clock),]


int <- read.csv("Interaction_Results_25Aug2025.csv", header=T)

int2 <- merge(int, dis, by.x="disease", by.y="name")
names(int2)[c(1,10)] <- c("Dis_short","Disease")
...
int2$local_p <- int2$global_p <- NA

int2 <- int2[c(11,10,2:8,13,12,9)]

data1 <- rbind(data, int2)
...
ggsave("Figure1_28Aug2025.pdf", combined_plot, width = 20, height = 16)
...
ggsave("Figure2_28Aug2025.pdf", fig, , width = 12, height = 10)
